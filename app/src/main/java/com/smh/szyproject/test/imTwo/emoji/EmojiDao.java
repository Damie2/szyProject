package com.smh.szyproject.test.imTwo.emoji;
...
public class EmojiDao {
    ...
    public static String CopySqliteFileFromRawToDatabases(String SqliteFileName) throws IOException {
        ...
        //复制的话这里需要换成自己项目的包名
        File dir = new File("data/data/" + MyApplication.getApplication().getPackageName() + "/databases");
        ...
        if (!file.exists()) {
            try {
                file.createNewFile();
                inputStream = MyApplication.getApplication().getClass().getClassLoader().getResourceAsStream("assets/" + SqliteFileName);
